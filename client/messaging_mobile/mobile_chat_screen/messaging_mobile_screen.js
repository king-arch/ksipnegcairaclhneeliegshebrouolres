...
function changeUserStatus(status){
  Meteor.call('change_user_online_status',Session.get("userId"),status,function(error,result){
   ...
   Template.messanging_mobile_screen.onRendered(function(){
    var url = window.location.href;

        var new_url = url.split("/");
        url = new_url[new_url.length-1];
        url = Base64.decode(encrypted);

Session.set("msgid_forright",url);
...
Template.messanging_mobile_screen.onCreated(function(){
...
Template.messanging_mobile_screen.events({
 ...
 //       // alert(altura);
  ...
  'click .send_person_to_msg_page': function(){
    Session.setPersistent("msgid_forright",this.user_id);  
    show($('#seconddiv'));
    hide($('#firstdiv'));
     setTimeout(function() {
      $("#message_container").animate({ scrollTop: $('#message_container').prop("scrollHeight")}, 1);
...
Session.setPersistent("msgid_forright",show_id);
 ...
 if(check_chatroom[0]){
    ...
    Template.messanging_mobile_screen.__helpers["msg_rightbox"]();
    Template.messanging_mobile_screen.__helpers["show_message_array"]();

},
'click #back_button':function(event){
	Session.clear("msgid_forright");
	Router.go("/Messaging_mobile");
...
if(currently_typing_multi == 0 || currently_typing_multi ==""){
 ...
 Template.messanging_mobile_screen.helpers({
...
// console.log('dummy: '+dummy);
  ...
  msg_rightbox(){
   ...
   var show_details = connection_details[0];
   // alert(JSON.stringify(show_details));                                            
   return connection_details;                                                      
  },                                                                              
